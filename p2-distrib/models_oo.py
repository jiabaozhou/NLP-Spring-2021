...
class Seq2SeqSemanticParser(nn.Module):
    def __init__(self, input_indexer, output_indexer, emb_dim, hidden_size, attention=False, embedding_dropout=0.2,
                 bidirect=True, teacher_forcing_prob=0.5):
        ...
        self.decoder = AttentionDecoder(emb_dim, hidden_size, len(output_indexer)) if attention else RNNDecoder(emb_dim,
                                                                                                                hidden_size,
                                                                                                                len(
                                                                                                                    output_indexer))
    ...
    def forward(self, x_tensor, inp_lens_tensor, y_tensor, out_lens_tensor, inp, batch_size):
        ...
        """
        # print(x_tensor.shape, inp_lens_tensor.shape)
        ...
        if self.attention:
            ...
            # print(enc_output.shape, context_mask.shape, h_t[0].shape, y_tensor.shape)
            hn, cn = h_t
            # print(hn.shape)
            ...
            for i in range(out_lens_tensor):
                # print("we're at ", i)
                output, hn = self.decoder(inp, hn, enc_output, context_mask)
                # print(i, output.shape, hn.shape)
                is_teacher_forced = True if \
                    random.choices([1, 0], [self.teacher_forcing_prob, 1 - self.teacher_forcing_prob])[
                        0] == 1 else False
                if is_teacher_forced:
                    ...
                    inp = torch.argmax(output, dim=2).squeeze(0)
                # print(inp, inp.shape, is_teacher_forced)
                outputs[i] = output

            return outputs
        else:
            output, context_mask, h_t = self.encode_input(x_tensor, inp_lens_tensor)
            # print('yes')
            hn, cn = h_t
            # print(hn.shape)
            outputs = torch.zeros(out_lens_tensor, batch_size, len(self.output_indexer))
            for i in range(out_lens_tensor):
                # print(i, input.shape, hn.shape, cn.shape)
                # print(i, input.shape, hn.shape)
                output, hn, cn = self.decoder(inp, hn, cn)
                # print(output.shape)
                is_teacher_forced = True if \
                random.choices([1, 0], [self.teacher_forcing_prob, 1 - self.teacher_forcing_prob])[
                    0] == 1 else False
                # print(is_teacher_forced)
                if is_teacher_forced:
                    inp = y_tensor[:, i]
                    # print('teach force', input.shape)
                else:
                    # print('output', output)
                    inp = torch.argmax(output, dim=2).squeeze()
                    # print('not teach forced', input)
    ...
    def decode(self, test_data: List[Example]) -> List[List[Derivation]]:
        ...
        for i, ex in enumerate(test_data):
            ...
            while token != "<EOS>" and idx < 70:
                ...
                output, hn = self.decoder(input, hn, enc_outputs, context_mask) if self.attention else self.decoder(input, hn, cn)
...
def train_model_encdec(train_data: List[Example], dev_data: List[Example], input_indexer, output_indexer,
    ...
    embedding_size = 300
    epochs = 10
    model = Seq2SeqSemanticParser(input_indexer, output_indexer, embedding_size, hidden_size, attention)
    initial_lr = 0.001
    optm = optim.Adam(list(model.parameters()) + list(model.encoder.parameters()) + list(model.decoder.parameters()),
                      lr=initial_lr)
    ...
    for epoch in range(epochs):
        ...
        while batch_index < len(all_train_input_data):
            ...
            # print("inp", inp_lens_tensor)
            # print("out", out_lens_tensor)
            inp = torch.full([batch_size], output_indexer.index_of("<SOS>")).long()

            # print(x_tensor.shape, y_tensor.shape, inp_lens_tensor.shape, input.shape)
            outputs = model(x_tensor, inp_lens_tensor, y_tensor, output_max_len, inp, batch_size)

            # y_tensor = y_tensor.view(batch_size*output_max_len)
            # print(outputs.shape, y_tensor.shape)
            # input()
            loss = model.loss_func(outputs.view(batch_size*output_max_len, len(output_indexer)), y_tensor.view(batch_size*output_max_len))
            loss.backward()
            optm.step()
